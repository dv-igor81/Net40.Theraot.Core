...
namespace Net40.TaskDemo
{
    public static class TaskExWhenAnyTests
    {
        private static void ReceiveContinueWith(Task<int> prevTask)
        {
            Console.WriteLine("++++++++++++ ReceiveContinueWith ++++++++++++");
        }
        
        public static async ValueTask<int> RunTest(CancellationToken ct)
        {
            Task<int> taskReceive = ReceiveAsync(ct);
            await taskReceive.ContinueWith(ReceiveContinueWith);
        ...
        private static Task<int> ChanelAsync(CancellationToken ct)
        {
            ...
            Task<int> task = TaskEx.Run(ssh.Execute);
