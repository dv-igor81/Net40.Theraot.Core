...
public sealed class ExceptionDispatchInfo
{
	...
	private ExceptionDispatchInfo(Exception exception)
	{
		...
		if (_stackTrace != null)
		{
			_stackTrace = _stackTrace?.ToString() + Environment.NewLine + "---End of stack trace from previous location where exception was thrown ---" + Environment.NewLine;
		}
		else
		{
	...
	private static FieldInfo GetFieldInfo()
	{
		...
		_remoteStackTraceString = typeof(Exception).GetField("_remoteStackTraceString", BindingFlags.Instance | BindingFlags.NonPublic) ?? typeof(Exception).GetField("remote_stack_trace", BindingFlags.Instance | BindingFlags.NonPublic);
		return _remoteStackTraceString;
	}
