...
public static class TaskExEx
{
	private sealed class WaitHandleCancellableTaskCompletionSourceManager
	{
		...
		{
			WaitHandleCancellableTaskCompletionSourceManager result = new WaitHandleCancellableTaskCompletionSourceManager(cancellationToken, taskCompletionSource);
			result._registeredWaitHandle[0] = ThreadPool.RegisterWaitForSingleObject(waitHandle, result.CallbackWithoutTimeout, null, -1, executeOnlyOnce: true);
		...
		{
			WaitHandleCancellableTaskCompletionSourceManager result = new WaitHandleCancellableTaskCompletionSourceManager(cancellationToken, taskCompletionSource);
			result._registeredWaitHandle[0] = ThreadPool.RegisterWaitForSingleObject(waitHandle, result.CallbackWithTimeout, null, millisecondsTimeout, executeOnlyOnce: true);
			cancellationToken.Register(delegate
			{
				result.Unregister();
			});
	...
	private sealed class WaitHandleTaskCompletionSourceManager
	{
		...
		{
			WaitHandleTaskCompletionSourceManager waitHandleTaskCompletionSourceManager = new WaitHandleTaskCompletionSourceManager(taskCompletionSource);
			waitHandleTaskCompletionSourceManager._registeredWaitHandle[0] = ThreadPool.RegisterWaitForSingleObject(waitHandle, waitHandleTaskCompletionSourceManager.CallbackWithoutTimeout, null, -1, executeOnlyOnce: true);
		}
		...
		{
			WaitHandleTaskCompletionSourceManager waitHandleTaskCompletionSourceManager = new WaitHandleTaskCompletionSourceManager(taskCompletionSource);
			waitHandleTaskCompletionSourceManager._registeredWaitHandle[0] = ThreadPool.RegisterWaitForSingleObject(waitHandle, waitHandleTaskCompletionSourceManager.CallbackWithTimeout, null, millisecondsTimeout, executeOnlyOnce: true);
		}
	...
	internal static Task FromWaitHandleInternal(WaitHandle waitHandle, CancellationToken cancellationToken)
	{
		...
		WaitHandleCancellableTaskCompletionSourceManager.CreateWithoutTimeout(waitHandle, cancellationToken, taskCompletionSource);
		return taskCompletionSource.Task;
	}
	...
	internal static Task<bool> FromWaitHandleInternal(WaitHandle waitHandle, int millisecondsTimeout)
	{
		...
		{
			WaitHandleTaskCompletionSourceManager.CreateWithTimeout(waitHandle, taskCompletionSource, millisecondsTimeout);
		}
		return taskCompletionSource.Task;
	}
	...
	internal static Task<bool> FromWaitHandleInternal(WaitHandle waitHandle, int millisecondsTimeout, CancellationToken cancellationToken)
	{
		...
		{
			WaitHandleCancellableTaskCompletionSourceManager.CreateWithTimeout(waitHandle, cancellationToken, taskCompletionSource, millisecondsTimeout);
		}
		return taskCompletionSource.Task;
	}
