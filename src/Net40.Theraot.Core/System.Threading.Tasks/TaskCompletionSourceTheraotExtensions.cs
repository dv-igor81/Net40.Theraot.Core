...
public static class TaskCompletionSourceTheraotExtensions
{
	private static class TrySetCanceledCachedDelegate<T>
	{
		...
		public static Func<TaskCompletionSource<T>, CancellationToken, bool> TrySetCanceledCached => _trySetCanceledCached ?? (_trySetCanceledCached = CreateTrySetCanceledDelegate());

		private static Func<TaskCompletionSource<T>, CancellationToken, bool> CreateTrySetCanceledDelegate()
		{
			MethodInfo method = typeof(TaskCompletionSource<T>).GetMethod("TrySetCanceled", new Type[1] { typeof(CancellationToken) });
			if (method != null)
			{
				return (Func<TaskCompletionSource<T>, CancellationToken, bool>)MethodInfoTheraotExtensions.CreateDelegate(method, typeof(Func<TaskCompletionSource<T>, CancellationToken, bool>));
			}
			new TraceSource("Theraot.Core").TraceEvent(TraceEventType.Warning, 1, "TaskCompletionSource<T>.TrySetCanceled(CancellationToken): fallback to overload without CancellationToken.");
			...
			Func<TaskCompletionSource<T>, bool> trySetCanceled = (Func<TaskCompletionSource<T>, bool>)MethodInfoTheraotExtensions.CreateDelegate(method, typeof(Func<TaskCompletionSource<T>, bool>));
