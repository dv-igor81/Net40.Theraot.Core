...
[DebuggerNonUserCode]
public class WeakNeedle<T> : IEquatable<WeakNeedle<T>>, IRecyclable, ICacheNeedle<T>, INeedle<T>,
    IReadOnlyNeedle<T>, IPromise<T>, IPromise where T : class
{
    ...
    public override string ToString()
    {
            ...
            T target;
            return (_handle != null && _handle.TryGetTarget(out target))
                ? (target.ToString() ?? "<?>")
                : "<Dead Needle>";
        }
    ...
    private static bool EqualsExtractedExtracted(WeakNeedle<T> left, WeakNeedle<T> right)
    {
            ...
            {
                return EqualityComparer<Exception>.Default.Equals(exception, exception2);
            }
