...
{
    [ThreadStatic] private static HashSet<UniqueId> _guard;
    ...
    private ThreadSafeQueue<Action> WorkQueue =>
        TypeHelper.LazyCreate(ref _workQueue, () => new ThreadSafeQueue<Action>());
    ...
    public IDisposable TryEnter(out bool didEnter)
    {
            ...
            {
                IDisposable disposable = DisposableAkin.Create(delegate { Leave(Id); });
                result = disposable;
            }
