...
internal sealed class BucketCore : IEnumerable<object>, IEnumerable, ISerializable
{
    ...
    {
            if (!(info.GetValue("childFactory", typeof(Func<object>)) is Func<object> childFactory) ||
    ...
    {
            _childFactory = ((level == 1)
                ? FuncHelper.GetDefaultFunc<object>()
                : ((Func<object>)(() => new BucketCore(_level - 1))));
            _level = level;
            _arrayFirst = ArrayReservoir<object>.GetArray(256);
            _arraySecond = ArrayReservoir<object>.GetArray(256);
            _arrayUse = ArrayReservoir<int>.GetArray(256);
    ...
    public bool Do(int index, DoAction callback)
    {
            ...
            return Do(ref array3[num], ref array[num], ref array2[num],
                (_level == 1)
                    ? callback
                    : ((DoAction)delegate(ref object target)
                    {
    ...
    public bool DoMayDecrement(int index, DoAction callback)
    {
            ...
            int num = SubIndex(index);
            return DoMayDecrement(ref array3[num], ref array[num], ref array2[num],
                (_level == 1)
                    ? callback
                    : ((DoAction)delegate(ref object target)
                    {
    ...
    public bool DoMayIncrement(int index, DoAction callback)
    {
            ...
            int num = SubIndex(index);
            return DoMayIncrement(ref array3[num], ref array[num], ref array2[num], childFactory,
                (_level == 1)
                    ? callback
                    : ((DoAction)delegate(ref object target)
                    {
    ...
    private static bool DoMayIncrement(ref int use, ref object first, ref object second, Func<object> factory,
        DoAction callback)
    {
            try
            {
    ...
    private IEnumerable<object> PrivateEnumerableRange(int indexFrom, int indexTo, int startSubIndex,
        int endSubIndex)
    {
            object[] arrayFirst = Volatile.Read(ref _arrayFirst);
            object[] arraySecond = Volatile.Read(ref _arraySecond);
            ...
            for (int subIndex = startSubIndex; subIndex < endSubIndex + 1; subIndex += step)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (object item in core.PrivateEnumerableRange(indexFrom, indexTo, subIndexFrom,
                                     subIndexTo))
                        {
                            yield return item;
                        }
    ...
    {
            return (int)(((uint)index >> 8 * (_level - 1)) & 0xFF);
        }
}