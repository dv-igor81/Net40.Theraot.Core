...
public sealed class FixedSizeBucket<T> : IBucket<T>, IEnumerable<T>, IEnumerable, ISerializable
{
	...
	{
		if (!(info.GetValue("count", typeof(int)) is int val) || !(info.GetValue("contents", typeof(object[])) is object[] array))
		{
	...
	{
		previous = default(T);
		object obj = Interlocked.CompareExchange(ref _entries[index], (item != null) ? ((object)item) : BucketHelper.Null, null);
		if (obj == null)
		{
			Interlocked.Increment(ref _count);
			return true;
	...
	internal bool InsertInternal(int index, T item)
	{
		...
		object obj = Interlocked.CompareExchange(ref entries[index], (item != null) ? ((object)item) : BucketHelper.Null, null);
		if (obj != null)
		{
			return false;
		}
	...
	{
		isNew = Interlocked.Exchange(ref _entries[index], (item != null) ? ((object)item) : BucketHelper.Null) == null;
		if (isNew)
		{
			Interlocked.Increment(ref _count);
		}
	...
	internal bool UpdateInternal(int index, Func<T, T> itemUpdateFactory, Predicate<T> check, out bool isEmpty)
	{
		...
		if (obj != null)
		{
			...
			if (check(val))
			{
				...
				T val3 = val2;
				obj2 = Interlocked.CompareExchange(ref location, (val3 != null) ? ((object)val3) : BucketHelper.Null, obj);
				result = obj == obj2;
			}
		}
